...
# 🌟 [업데이트] 맨 앞에 '종합 랭킹' 탭 추가
tabs = st.tabs(["🏆 종합 랭킹"] + subjects)

# --- 1️⃣ 종합 랭킹 메인 페이지 ---
with tabs[0]:
    st.subheader("🏆 전체 과목 수강률 종합 랭킹")
    st.write("각 과목 멘토님들이 데이터를 업로드하면 실시간으로 랭킹이 변동됩니다.")
    ...
    # 랭킹 1: 평균 수강률 높은 순 (좋은 랭킹)
    with col1:
        st.markdown("### 📈 전체 평균 수강률 랭킹 (이수율 순)")
        ...
        for i in range(len(subjects)):
            if i < len(sorted_by_comp):
                ...
                # 데이터가 없는 빈 랭킹 (점선 박스로 예쁘게 표시)
                st.markdown(f"""
                    <div style='padding: 12px; border: 2px dashed #ccc; border-radius: 8px; color: #888; margin-bottom: 12px; background-color: #fafafa;'>
                        <b>{i+1}위</b> | ⬜ 아직 자료가 없어요
                    </div>
                """, unsafe_allow_html=True)
                
    # 랭킹 2: 1강도 수강 안 한 학생 많은 순 (위험도 랭킹)
    with col2:
        st.markdown("### 🚨 미수강자 위험도 랭킹 (0강 수강 비율 높은 순)")
...
for i, subject in enumerate(subjects):
    with tabs[i+1]: # 0번이 종합랭킹이므로 1번 탭부터 시작
        ...
        if os.path.exists(file_path):
            # 🌟 [업데이트] 저장해둔 날짜가 있으면 대시보드 우측 상단에 예쁘게 표시
            if os.path.exists(date_path):
                with open(date_path, "r", encoding="utf-8") as f:
                    saved_date = f.read()
                st.markdown(f"<p style='text-align: right; color: #E67E22; font-weight: bold;'>🕒 기준일: {saved_date}</p>", unsafe_allow_html=True)
