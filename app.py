...
<style>
    ...
    /* 🌟 [수정 완료] 모바일 환경 제목 글자 크기 더 작게 & 한 줄 고정 */
    @media (max-width: 768px) {
        .main-title { font-size: 18px; white-space: nowrap; letter-spacing: -0.5px; } /* Reduce from 21px to 18px */
...
# 🌟 타이틀 영역 (icon removed as requested, left-aligned)
st.markdown("""
...
# 🌟 Updated to include icon, using previously correct function signature
def create_card(icon, title, value, desc, badge="", progress=None, color="#2980B9"):
    badge_html = f'<span style="font-size:12px; padding:3px 8px; border-radius:12px; font-weight:700; background-color:rgba(128,128,128,0.1); color:{color};">{badge}</span>' if badge else ""
    progress_html = f'<div style="width:100%; background-color:rgba(128,128,128,0.2); height:6px; border-radius:4px; margin-top:10px;"><div style="width:{progress:.1f}%; background-color:{color}; height:100%; border-radius:4px;"></div></div>' if progress is not None else ""
...
for i, subject in enumerate(subjects):
    with tabs[i+1]:
        ...
        if os.path.exists(file_path):
            ...
            # 🌟 [수정 완료] 보내주신 시안 캡처본에 없던 이모티콘을 다시 넣었습니다!
            c1 = create_card("👥", "전체 수강생", f"{total_cnt}명", "이번 학기 등록 인원", "Total", color="var(--text-color)")
            c2 = create_card("✅", "안정권 학생", f"{len(high_df)}명", "10강 이상 수강 완료", "10강↑", color="#27AE60")
            c3 = create_card("🚨", "전면 미수강", f"{len(zero_df)}명", "수강 이력 없음 (밀착관리)", "0강", color="#E74C3C")
            c4 = create_card("📊", "평균 수강률", f"{avg_rate:.1f}%", "전체 출석 ÷ (인원×15강)", progress=avg_rate, color="#2980B9")
            c5 = create_card("🎯", "안정권 비율", f"{high_rate:.1f}%", "안정권 학생 ÷ 전체 수강생", progress=high_rate, color="#27AE60")
            c6 = create_card("⚠️", "미수강 비율", f"{zero_rate:.1f}%", "미수강 학생 ÷ 전체 수강생", progress=zero_rate, color="#E74C3C")
            ...
            # 🌟 [수정 완료] 하단 탭 제목에도 이모티콘을 다시 넣었습니다!
