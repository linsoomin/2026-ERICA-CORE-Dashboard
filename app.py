...
if not st.session_state['authenticated']:
    ...
    /* ── 배경 (라이트) ── */
    .stApp { background: #eef2ff !important; min-height: 100vh; }
    ...
    .deco-circle {
        ...
        filter: blur(80px); opacity: 0.20;
    }
    .deco-c1 { width:380px;height:380px; background:radial-gradient(circle,#3b82f6,transparent); top:-100px;left:-100px; }
    .deco-c2 { width:300px;height:300px; background:radial-gradient(circle,#8b5cf6,transparent); bottom:-80px;right:-80px; }
    @media (prefers-color-scheme: dark) {
        .deco-circle { opacity: 0.15; }
    }
    ...
    .login-card {
        ...
        box-shadow: 0 8px 48px rgba(37,99,235,0.10), 0 1px 4px rgba(0,0,0,0.04);
    ...
    /* ── 타이틀 ── */
    .login-title {
        font-size: 28px; font-weight: 900; letter-spacing: -1px; line-height: 1.2;
        margin: 0 0 6px;
        background: linear-gradient(135deg,#2563eb 0%,#7c3aed 100%);
        -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;
    }
    .login-sub {
        font-size: 13px; color: #94a3b8; font-weight: 500; margin-bottom: 28px;
    }
    @media (prefers-color-scheme: dark) {
    ...
    @media (prefers-color-scheme: dark) {
        .login-hr { background: linear-gradient(90deg, transparent, #1e2636, transparent); }
    }

    /* ── 인풋 라벨 ── */
    div[data-testid="stTextInput"] label {
        font-size: 11.5px !important; font-weight: 700 !important;
        color: #64748b !important; letter-spacing: 0.08em !important;
    ...
    @media (prefers-color-scheme: dark) {
        div[data-testid="stTextInput"] > div:first-child {
            border-color: #1e2636 !important; background: #0d1117 !important;
        }
        div[data-testid="stTextInput"] > div:first-child:focus-within {
    ...
    div[data-testid="stFormSubmitButton"] button:hover {
        ...
        box-shadow: 0 7px 24px rgba(37,99,235,0.38) !important;
    }
    div[data-testid="stFormSubmitButton"] button:active { transform: translateY(0) !important; }

    /* ── 푸터 ── */
    .login-footer {
        text-align: center; font-size: 11.5px; color: #b0bac8;
        margin-top: 30px; font-weight: 500;
    }
    @media (prefers-color-scheme: dark) { .login-footer { color: #2a3245; } }
    </style>
    ...
    with col:
        ...
        <div class="login-wrap">
          <div class="login-card">
            <div><span class="login-badge">🐣 CORE Mentor Login</span></div>
            <div class="login-title">2026 CORE<br>수강률 대시보드</div>
            <div class="login-sub">한양대학교 ERICA 기초과학교육센터</div>
        ...
        # 폼을 카드 위에 오버레이
        st.markdown("<div style='margin-top:-340px; padding:0 4px;'>", unsafe_allow_html=True)
        ...
        st.markdown('<div class="login-footer">© 2026 한양대학교 ERICA 기초과학교육센터</div>', unsafe_allow_html=True)
...
.hero-sub { color: rgba(255,255,255,0.80) !important; font-size: 13px; margin-top: 8px; font-weight: 500; }

.section-title { font-size: 17px; font-weight: 700; color: var(--text-color); margin-bottom: 12px; margin-top: 15px; }
button[data-baseweb="tab"] * { font-size: 14px !important; font-weight: 700 !important; }
...
.metric-label { font-size: 13px; font-weight: 600; opacity: 0.75; }
.metric-value { font-size: 26px; font-weight: 900; line-height: 1; }
.metric-sub { font-size: 12px; color: gray; font-weight: 600; }
.progress-wrap { width: 100%; height: 5px; border-radius: 4px; background: rgba(128,128,128,0.12); margin-top: 12px; overflow: hidden; }
...
.rank-item {
    ...
    border: 1px solid rgba(128,128,128,0.15);
    margin-bottom: 9px; transition: 0.2s ease;
}
.rank-item:hover { transform: translateX(3px); border-color: #2563eb; }
.rank-left { display: flex; align-items: center; gap: 12px; }
.rank-num { width: 30px; height: 30px; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-weight: 900; font-size: 12px; color: white; flex-shrink: 0; }
.rank-name { font-size: 14px; font-weight: 800; color: var(--text-color); }
.rank-desc { font-size: 11px; color: gray; margin-top: 2px; }
.rank-score { font-size: 17px; font-weight: 900; }
...
.chip-gray { background: rgba(128,128,128,0.08); color: var(--text-color); }
...
[data-testid="stFileUploaderDropzone"] { padding: 5px 15px !important; background-color: transparent !important; border: 1px dashed rgba(128,128,128,0.4) !important; border-radius: 8px; }
.stButton>button { width: 100%; font-weight: 700; border-radius: 10px; border-color: #2563eb; color: #2563eb; }
</style>
""", unsafe_allow_html=True)
...
def create_card(icon, title, value, desc, badge="", progress=None, color="#2563eb"):
    ...
    val_color = "var(--text-color)" if color == "gray" else color
    return f'<div class="metric-card"><div class="metric-header"><span class="metric-label">{icon} {title}</span>{b_html}</div><div class="metric-value" style="color:{val_color};">{value}</div>{p_html}<div class="metric-sub" style="margin-top:8px;">{desc}</div></div>'

def create_rank_item(rank, name, value_text, tone="blue", desc=""):
    tone_map = {
        "blue":  ("#3b82f6","linear-gradient(135deg,#3b82f6,#2563eb)"),
        "green": ("#22c55e","linear-gradient(135deg,#22c55e,#16a34a)"),
        "red":   ("#ef4444","linear-gradient(135deg,#ef4444,#dc2626)"),
        "amber": ("#f59e0b","linear-gradient(135deg,#f59e0b,#d97706)")
    }
    tc, bg = tone_map.get(tone, tone_map["blue"])
    return f'<div class="rank-item"><div class="rank-left"><div class="rank-num" style="background:{bg};">{rank}</div><div><div class="rank-name">{name}</div><div class="rank-desc">{desc}</div></div></div><div class="rank-score" style="color:{tc};">{value_text}</div></div>'

tabs = st.tabs(["🏆 종합 랭킹"] + [f"{SUBJECT_ICONS.get(s,'📘')} {s}" for s in subjects])

with tabs[0]:
    ranking_data, all_dept_data = [], []
    ...
    for subj in subjects:
        ...
        if os.path.exists(fp):
            try:
                ...
                t = len(df)
                if t > 0:
                    h, z = len(df[df['출석'] >= 10]), len(df[df['출석'] == 0])
                    ranking_data.append({'과목': subj, '이수율': h/t*100, '미수강비율': z/t*100, '인원': t})
                    total_stu += t; total_high += h; total_zero += z
                if '학과' in df.columns:
    ...
    sum_cards = "".join([
        ...
        create_card("🟢","안정권 비율",f"{avg_comp:.1f}%","10강 이상 (평균)",progress=avg_comp,color="#16a34a"),
        create_card("🚨","미수강 비율",f"{avg_zero:.1f}%","0강 (평균)",progress=avg_zero,color="#dc2626"),
    ])
    ...
    with col1:
        ...
        for i, item in enumerate(sorted(ranking_data, key=lambda x: x['이수율'], reverse=True), 1):
            box += create_rank_item(i, item['과목'], f"{item['이수율']:.1f}%", "green" if i==1 else "blue", f"전체 {item['인원']}명 중 안정권 비율")
        box += "</div>"
    ...
    with col2:
        ...
        for i, item in enumerate(sorted(ranking_data, key=lambda x: x['미수강비율'], reverse=True), 1):
            box += create_rank_item(i, item['과목'], f"{item['미수강비율']:.1f}%", "red" if i==1 else "amber", f"전체 {item['인원']}명 중 0강 비율")
        box += "</div>"
        st.markdown(box, unsafe_allow_html=True)

    st.markdown("<div class='section-title' style='margin-top:30px;'>🏫 학과별 수강 현황 (전 과목 종합)</div>", unsafe_allow_html=True)
    if all_dept_data:
        combined = pd.concat(all_dept_data).dropna(subset=['학과'])
        dept_stats = combined.groupby('학과').agg(
            수강생수=('출석','count'),
            평균수강률=('출석', lambda x: (x.mean()/15)*100),
            안정권비율=('출석', lambda x: (len(x[x>=10])/len(x))*100)
        ).reset_index().sort_values('평균수강률', ascending=False).reset_index(drop=True)
        top = dept_stats.iloc[0]
        st.markdown(f"""
        <div class="info-band">
            <span class="inline-chip chip-blue">🏅 최고 학과: {top['학과']}</span>
            <span class="inline-chip chip-gray">평균 수강률 {top['평균수강률']:.1f}%</span>
...
for i, subject in enumerate(subjects):
    with tabs[i+1]:
        ...
        with st.expander("엑셀 업데이트"):
            ...
            if uploaded_file and st.button("반영하기", key=f"btn_{subject}"):
                try:
                    raw = pd.read_excel(uploaded_file, header=None, nrows=1)
                    dm = re.search(r'\d{4}-\d{2}-\d{2}', str(raw.iloc[0,0]))
                    clean_date = dm.group(0) if dm else datetime.datetime.now().strftime('%Y-%m-%d')
                    with open(date_path,"w",encoding="utf-8") as f: f.write(clean_date)
                    pd.read_excel(uploaded_file, header=3).to_csv(file_path, index=False)
                    st.success("완료!"); st.rerun()
                except Exception as e: st.error(f"오류: {e}")

        if os.path.exists(file_path):
            s_date = open(date_path,"r",encoding="utf-8").read() if os.path.exists(date_path) else "날짜 정보 없음"
            ...
            cards = "".join([
                ...
                create_card("✅","안정권 학생",f"{len(h_df)}명","10강 이상",badge="10강↑",color="#16a34a"),
                create_card("🚨","전면 미수강",f"{len(z_df)}명","수강 없음",badge="0강",color="#dc2626"),
                create_card("📊","평균 수강률",f"{avg_r:.1f}%","15강 기준",progress=avg_r,color="#2563eb"),
                create_card("🎯","안정권 비율",f"{h_r:.1f}%","전체 대비",progress=h_r,color="#16a34a"),
                create_card("⚠️","미수강 비율",f"{z_r:.1f}%","전체 대비",progress=z_r,color="#dc2626"),
            ])
