...
if not st.session_state['authenticated']:
    ...
    # 3등분 해서 가운데 배치 (비율 1 : 1.2 : 1)
    col1, col2, col3 = st.columns([1, 1.2, 1]) 
    
    with col2:
        st.markdown("<h2 style='text-align: center; color: #2980B9; margin-bottom: 30px;'>🔒 CORE Mentor Login</h2>", unsafe_allow_html=True)
        ...
        if st.button("로그인", use_container_width=True):
            if input_id == "2026ericacore" and input_pw == "2026ericacore@@":
                ...
                st.rerun()  # 성공 시 화면 새로고침하여 대시보드 진입
            else:
                st.error("❌ 아이디 또는 비밀번호가 일치하지 않습니다.")
                
    # 로그인 성공 전까지는 절대 아래로 넘어가지 않음
    st.stop()  

# ==========================================
# 🏆 2. 여기서부터 대시보드 본문 (로그인 성공 시에만 보임)
# ==========================================
