...
st.markdown("""
<style>
    /* 🌟 [깐지 타이틀] 젠스파크 스타일 히어로 섹션 */
    .hero {
        position: relative; overflow: hidden; padding: 25px 25px; border-radius: 20px;
        background: linear-gradient(135deg, rgba(37,99,235,0.9) 0%, rgba(29,78,216,0.9) 45%, rgba(124,58,237,0.8) 100%);
    ...
    .hero-badge {
        ...
        border: 1px solid rgba(255,255,255,0.2); color: white; padding: 5px 12px;
        border-radius: 999px; font-size: 11px; font-weight: 700; margin-bottom: 12px;
    }
    .hero-title { color: white; font-size: 32px; font-weight: 900; letter-spacing: -1px; margin: 0; line-height: 1.1; }
    .hero-sub { color: rgba(255,255,255,0.85); font-size: 14px; margin-top: 8px; font-weight: 500; }
    ...
    for subj in subjects:
        ...
        if os.path.exists(file_path):
            try:
                ...
                if '학과' in df.columns: all_dept_data.append(df[['학과', '출석']])
            except: pass
...
st.markdown("<br><div style='text-align: center; color: gray; font-size: 12px;'>© 2026 한양대학교 ERICA 기초과학교육센터</div>", unsafe_allow_html=True)
